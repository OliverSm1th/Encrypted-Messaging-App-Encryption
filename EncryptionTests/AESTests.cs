...
using System;
using System.Text;
using static Encryption_Algorithms.Functions;
using Encryption_Algorithms;
using System.Linq;
...
namespace AESTest
{
    ...
    [TestClass]
    public class EncryptionCBCTests
    {
        //public static void Main(string[] args)
        //{
        //    MonteCarloTest("9dc2c84a37850c11699818605f47958c", "256953b2feab2a04ae0180d8335bbed6", "2e586692e647f5028ec6fa47a55a2aab");
        //}
        public static void MonteCarloTest(string intial_key, string initial_iv, string initial_pt)
        {
            int keylen = intial_key.Length * 4;
            Console.WriteLine(keylen);
            AES aes = new AES(keylen);
            byte[][] key = new byte[100][];
            byte[][] iv = new byte[100][];
            byte[][] pt = new byte[1001][];
            byte[][] ct = new byte[1000][];
            key[0] = HexToByteArr(intial_key); iv[0] = HexToByteArr(initial_iv); pt[0] = HexToByteArr(initial_pt);
            for(int i=0; i<99; i++)
            {
                Console.WriteLine($"KEY = {ByteArrToHex(key[i])}");
                Console.WriteLine($"IV = {ByteArrToHex(iv[i])}");
                Console.WriteLine($"PT = {ByteArrToHex(pt[0])}");
                int encryptNum = 0;
                for(int j=0; j<=999; j++)
                {
                    if (j == 0)
                    {
                        ct[j] = aes.EncryptCBC(key[i], pt[j], iv[i]);
                        pt[j + 1] = iv[i];
                    }
                    else
                    {
                        ct[j] = aes.EncryptECB(key[i], pt[j]);
                        pt[j + 1] = ct[j - 1];
                    }
                    encryptNum++;
                }
                Console.WriteLine($"CT = {ByteArrToHex(ct[999])}");
                if ( keylen == 128)
                {
                    key[i + 1] = ByteXOR(key[i], ct[99]);
                }
                if (keylen == 192)
                {
                    key[i + 1] = ByteXOR(key[i], ByteOR(ct[998].Skip(999 - 8).Take(8).ToArray(), ct[999]));
                }
                if (keylen == 256)
                {
                    key[i + 1] = ByteXOR(key[i], ByteOR(ct[998], ct[999]));
                }
                iv[i + 1] = ct[999];
                pt[0] = ct[998];
            }
        ...
        private static Byte[] ByteXOR(Byte[] Word1, Byte[] Word2)
        {
            if (Word1.Length != Word2.Length) { Console.WriteLine($"Invalid bytes given {Word1.Length} vs {Word2.Length}"); return new byte[4]; }
        ...
        private static Byte[] ByteOR(Byte[] Word1, Byte[] Word2)
        {
            if (Word1.Length != Word2.Length) { Console.WriteLine($"Invalid bytes given {Word1.Length} vs {Word2.Length}"); return new byte[4]; }
            Byte[] result = new byte[Word1.Length];
            for (int i = 0; i < Word1.Length; i++)
